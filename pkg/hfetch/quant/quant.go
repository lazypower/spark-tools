// Package quant derives a model's serving quantization format from its
// HuggingFace config files, so callers can report it without downloading the
// weights. It interprets metadata only — it never performs quantization math
// (see specs/01-hfetch.md §3, §14.7).
package quant
...
// Info is the detected quantization of a model. A nil *Info means no
...
func Parse(configJSON, hfQuantJSON, gptqJSON []byte) *Info {
	...
	if len(gptqJSON) > 0 {
		...
		if json.Unmarshal(gptqJSON, &gc) == nil && gc.Bits > 0 {
			return &Info{Method: "gptq", Algo: fmt.Sprintf("GPTQ-Int%d", gc.Bits)}
		}
	}
	...
	if len(configJSON) > 0 {
		...
		if json.Unmarshal(configJSON, &cfg) == nil && cfg.QuantizationConfig != nil {
			...
			info := &Info{Method: qc.QuantMethod}
			switch {
			case qc.Bits > 0:
